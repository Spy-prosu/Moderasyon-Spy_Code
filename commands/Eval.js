...
module.exports.run = async (client, message, args) => {
  ...
  try {
      ...
      if (codein.length < 1) return m.channel.send(`Bir kod girmelisin.`)//Spy Code
      if (typeof code !== 'string')    
        code = require('util').inspect(code, { depth: 0 });
    
      if(code.includes(process.env.TOKEN)) return m.reply("sg tokeni başka yöntemle al")//Spy Code
    ...
    .setColor('RED')
    .addField('Kod', `\`\`\`js\n${codein.length > 1024 ? "Karakter aşımı!" : codein}\`\`\``)//Spy Code
